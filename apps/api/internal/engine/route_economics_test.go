...
func TestCrewCostScalesWithCapacity(t *testing.T) {
	...
	small := crewCostFor(base, 90, defaultCrewScale())  // regional ~0.5x
	mid := crewCostFor(base, 180, defaultCrewScale())   // anchored at 1.0x
	large := crewCostFor(base, 360, defaultCrewScale()) // widebody ~2.0x
	huge := crewCostFor(base, 900, defaultCrewScale())  // clamped at 2.5x
	...
	if crewCostFor(base, 0, defaultCrewScale()) != base {
