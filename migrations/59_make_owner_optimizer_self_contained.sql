-- ============================================================================
-- MAKE OWNER OPTIMIZER SELF-CONTAINED
-- ============================================================================
-- Live environments may be missing one or more route helper functions even when
-- the optimizer migration itself is present. This migration inlines the helper
-- math so get_owner_route_optimizer() no longer depends on:
...
BEGIN
    ...
    fare_points AS (
        SELECT
            sp.*,
            ROUND((50.00 + (COALESCE(sp.distance_km, 0.0)::NUMERIC * 0.12)) * fare.multiplier, 2) AS ticket_price
    ...
    scored AS (
        SELECT
            ...
            fp.ticket_price,
            CASE
                WHEN COALESCE(fp.distance_km, 0.0) <= 0.0 OR COALESCE(fp.speed_kmh, 0) <= 0 THEN 0
                ELSE FLOOR(
                    168.0 /
                    NULLIF((COALESCE(fp.distance_km, 0.0) / fp.speed_kmh::DOUBLE PRECISION) + 1.0, 0.0)
                )::INT
            END AS weekly_flights,
            fp.economy_seats,
            fp.business_seats,
                ...
                LEAST(
                    ...
                    FLOOR(
                        ...
                        GREATEST(
                            ...
                            LEAST(
                                1.00,
                                0.55 + (((((COALESCE(fp.org_demand, 50) + COALESCE(fp.dst_demand, 50))::NUMERIC) / 2.0) / 100.0) * 0.45)
                            )
                        ) *
                        GREATEST(
                            0.00,
                            LEAST(
                                1.50,
                                1.5 - 0.8 * POWER(
                                    COALESCE(fp.ticket_price, 0.00) /
                                    NULLIF(50.00 + (COALESCE(fp.distance_km, 0.0)::NUMERIC * 0.12), 0.00),
                                    2
                                )
                            )
                        )
    ...
    SELECT
        ...
        ROUND((s.expected_passengers_per_flight * s.ticket_price)::NUMERIC, 2) AS revenue_per_flight,
        ROUND(((s.expected_passengers_per_flight * s.ticket_price) - s.direct_cost_per_flight)::NUMERIC, 2) AS contribution_per_flight,
        ROUND((((s.expected_passengers_per_flight * s.ticket_price) - s.direct_cost_per_flight) * s.weekly_flights)::NUMERIC, 2) AS weekly_contribution,
        ROUND(
            CASE
                WHEN s.acquisition_type = 'lease' THEN s.weekly_flights * 0.70
                ELSE s.weekly_flights * 0.50
            END::NUMERIC,
            2
...
'Returns ranked route, fare, and cabin-layout opportunities for the owner without depending on external route helper functions.';
