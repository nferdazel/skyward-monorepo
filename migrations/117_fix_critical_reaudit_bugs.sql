-- ============================================================================
-- Migration 117: Fix Critical Re-Audit Bugs
-- Bug 1: process_player_simulation_to_time — ambiguous column "cash"
-- Bug 2: bank_transactions INSERT silently fails (no checking account)
-- Bug 3: normalize_username trailing hyphen
-- ============================================================================

BEGIN;

-- ── 1. Fix normalize_username: strip leading/trailing hyphens ─────────────
...
-- ── 2. Create ensure_checking_account helper ─────────────────────────────
...
BEGIN
    ...
    VALUES (p_user_id, 'checking', (SELECT cash FROM users WHERE id = p_user_id))
...
-- ── 3. Fix process_player_simulation_to_time: ambiguous column + missing checking account ──
    ...
    IF v_elapsed_days >= 1.0 THEN
        ...
        IF v_net != 0 THEN
            ...
            SELECT ba.id, p_user_id,
                ...
                'Daily simulation net cash movement',
                p_target_game_time
...
-- ── 4. Fix process_all_bots_simulation_to_time: qualify column refs ───────
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            v_total_cost := v_fuel_cost + v_crew_cost + v_maint_cost;
            v_net := v_revenue + v_cargo_rev - v_total_cost - v_lease_cost;

            v_buffered_rev_accum := v_buffered_rev_accum + v_revenue;
            v_buffered_ops_accum := v_buffered_ops_accum + v_total_cost;
            v_buffered_lease_accum := v_buffered_lease_accum + v_lease_cost;
            v_buffered_cargo_accum := v_buffered_cargo_accum + v_cargo_rev;
        ...
        v_total_revenue := v_buffered_rev_accum + v_buffered_cargo_accum;
        v_total_cost_accum := v_buffered_ops_accum + v_buffered_lease_accum;
        v_net := v_total_revenue - v_total_cost_accum;
...
-- ── 5. Fix purchase_aircraft: ensure checking account before INSERT ───────
...
-- ── 6. Fix lease_aircraft: ensure checking account before INSERT ──────────
...
-- ── 7. Fix sell_aircraft: ensure checking account before INSERT ───────────
...
-- ── 8. Fix repair_aircraft: ensure checking account before INSERT ─────────
...
-- ── 9. Fix terminate_aircraft_lease: ensure checking account before INSERT ──
...
-- ── 10. Fix finance_aircraft (AI path): ensure checking account before INSERT ──
...
BEGIN
    ...
    IF v_actor_type = 'AI' THEN
        ...
        INSERT INTO fleet_aircraft (user_id, aircraft_model_id, tail_number, acquisition_type, condition, status, economy_seats, business_seats, first_class_seats)
        VALUES (p_user_id, p_aircraft_model_id, 'BOT-' || left(p_user_id::text, 4), 'finance', 100.00, 'active', v_economy_seats, v_business_seats, v_first_seats)
    ...
    INSERT INTO fleet_aircraft (user_id, aircraft_model_id, tail_number, acquisition_type, condition, status, economy_seats, business_seats, first_class_seats)
    VALUES (p_user_id, p_aircraft_model_id, generate_tail_number(COALESCE(v_hq_iata, 'CGK')), 'finance', 100.00, 'active', v_model.capacity, 0, 0)
    RETURNING id INTO v_fleet_id;
...
-- ── 11. Fix process_loan_payments: ensure checking account before INSERT ──
    ...
    FOR r_loan IN SELECT * FROM loans WHERE user_id = p_user_id AND status = 'active' ORDER BY taken_at ASC LOOP
        IF v_actor_type = 'AI' THEN
            IF v_cash >= r_loan.weekly_payment THEN
                ...
                UPDATE loans SET remaining_balance = remaining_balance - r_loan.weekly_payment WHERE id = r_loan.id;
...
-- ── 12. Fix process_aircraft_financing_payments: ensure checking account ──
...
-- ── 13. Fix take_loan: ensure checking account before INSERT (both paths) ──
...
BEGIN
    ...
    IF v_actor_type = 'AI' THEN
        ...
        INSERT INTO loans (user_id, principal, interest_rate, remaining_balance, weekly_payment, game_date_taken, status)
        VALUES (p_user_id, p_principal, v_interest_rate, v_total_repayable, v_weekly_payment, v_game_time, 'active')
...
-- ── 14. Fix repay_loan: ensure checking account before INSERT ─────────────
...
BEGIN
    ...
    SELECT ba.id, v_user_id, 'payment', -v_payment, ba.balance,
           ...
           'loan', p_loan_id, NOW()
    FROM bank_accounts ba WHERE ba.user_id = v_user_id AND ba.account_type = 'checking' LIMIT 1;

    INSERT INTO financial_ledger (user_id, transaction_type, category, amount, description, game_date)
    VALUES (v_user_id, 'expense', 'loan_payment', v_payment,
            CASE WHEN v_is_paid_off THEN 'Loan fully repaid' ELSE 'Loan partial repayment' END,
            NOW());
...
-- Test normalize_username (no trailing hyphen)
SELECT normalize_username('Test User!') AS test_normalize;

-- Test ensure_checking_account
SELECT ensure_checking_account(id) AS test_ensure_checking FROM users WHERE actor_type = 'REAL' LIMIT 1;

-- Verify world tick works
SELECT * FROM process_world_tick(NULL, 1);

-- Verify bank_transactions has entries
SELECT COUNT(*) AS bank_txn_count FROM bank_transactions;
