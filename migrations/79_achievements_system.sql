...
-- ACHIEVEMENTS SYSTEM
-- ============================================================================
-- Adds an achievements table and a check_achievements() function that is
-- called at the end of each game day in process_player_simulation_to_time.
--
-- Achievement types tracked:
--   first_flight      — First route established
--   fleet_10          — 10 aircraft in fleet
--   fleet_50          — 50 aircraft in fleet
--   millionaire       — Net worth exceeds $1M
--   multi_millionaire — Net worth exceeds $10M
--   hundred_million   — Net worth exceeds $100M
--   billionaire       — Net worth exceeds $1B
--   route_master      — 25 active routes
--   global_network    — Routes on 5+ continents
--   profit_streak     — 30 consecutive profitable days
--   market_leader     — #1 on leaderboard
--   survivor          — Recover from distress status
--   first_class       — Configure first-class seats
--   hub_builder       — 10+ routes from single airport
-- ============================================================================
...
CREATE POLICY achievements_select_own ON achievements FOR SELECT TO authenticated
...
-- ============================================================================
-- Evaluates all achievement conditions for a player and inserts any newly
-- unlocked achievements.  Uses ON CONFLICT DO NOTHING so repeated calls
-- are safe.  Returns rows for any achievements unlocked in the current call.
-- ============================================================================
...
DECLARE
    ...
    v_route_count INT;
    v_hub_routes INT;
    v_has_first_class BOOLEAN;
    v_distress_recovered BOOLEAN;
    v_achievement_count_before INT;
    v_achievement_count_after INT;
BEGIN
    -- Snapshot current achievement count so we can detect new unlocks
    SELECT COUNT(*) INTO v_achievement_count_before
    FROM achievements WHERE user_id = p_user_id;

    -- Get current state
    SELECT cash INTO v_cash FROM users WHERE id = p_user_id;
    SELECT COUNT(*) INTO v_fleet_count
    FROM user_fleet WHERE user_id = p_user_id AND status = 'active';
    SELECT COUNT(*) INTO v_route_count
    FROM user_routes WHERE user_id = p_user_id AND status = 'active';

    -- Net worth: cash + 70% residual value of active fleet
    SELECT v_cash + COALESCE(SUM(am.purchase_price * 0.7), 0)
    INTO v_net_worth
    FROM user_fleet uf
    JOIN aircraft_models am ON uf.aircraft_model_id = am.id
    WHERE uf.user_id = p_user_id AND uf.status = 'active';

    -- Hub routes: max routes departing from a single airport
    SELECT MAX(cnt) INTO v_hub_routes FROM (
        SELECT origin_iata, COUNT(*) AS cnt
        FROM user_routes
        WHERE user_id = p_user_id AND status = 'active'
        GROUP BY origin_iata
    ) sub;

    -- First class seats configured on any aircraft
    SELECT EXISTS(
        SELECT 1 FROM user_fleet
        WHERE user_id = p_user_id AND first_class_seats > 0
    ) INTO v_has_first_class;

    -- Survivor: recovered from distress (recovery_streak_days >= 3)
    SELECT COALESCE(recovery_streak_days, 0) >= 3
    ...
    IF v_route_count >= 1 THEN
        ...
        VALUES (p_user_id, 'first_flight', 'First Flight', 'Established your first route', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: fleet_10 ──
    IF v_fleet_count >= 10 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'fleet_10', 'Fleet Commander', 'Operate 10 aircraft', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: fleet_50 ──
    IF v_fleet_count >= 50 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'fleet_50', 'Air Fleet Admiral', 'Operate 50 aircraft', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: millionaire ──
    IF v_net_worth >= 1000000 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'millionaire', 'Millionaire', 'Net worth exceeds $1M', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: multi_millionaire ──
    IF v_net_worth >= 10000000 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'multi_millionaire', 'Multi-Millionaire', 'Net worth exceeds $10M', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: hundred_million ──
    IF v_net_worth >= 100000000 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'hundred_million', 'Aviation Mogul', 'Net worth exceeds $100M', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: billionaire ──
    IF v_net_worth >= 1000000000 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'billionaire', 'Aviation Billionaire', 'Net worth exceeds $1B', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: route_master ──
    IF v_route_count >= 25 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'route_master', 'Route Master', '25 active routes', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: hub_builder ──
    IF COALESCE(v_hub_routes, 0) >= 10 THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'hub_builder', 'Hub Builder', '10+ routes from a single airport', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: first_class ──
    IF v_has_first_class THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'first_class', 'First Class', 'Configured first-class cabin', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- ── Achievement: survivor ──
    IF v_distress_recovered THEN
        INSERT INTO achievements (user_id, achievement_type, achievement_name, description, game_date)
        VALUES (p_user_id, 'survivor', 'Survivor', 'Recovered from distress status', p_game_time)
        ON CONFLICT DO NOTHING;
    END IF;

    -- Return newly unlocked achievements (those that didn't exist before this call)
    SELECT COUNT(*) INTO v_achievement_count_after
    FROM achievements WHERE user_id = p_user_id;

    IF v_achievement_count_after > v_achievement_count_before THEN
        RETURN QUERY
        SELECT a.achievement_name, a.achievement_type
        FROM achievements a
        WHERE a.user_id = p_user_id
          AND a.id NOT IN (
              SELECT a2.id FROM achievements a2
              WHERE a2.user_id = p_user_id
              ORDER BY a2.unlocked_at ASC
              LIMIT v_achievement_count_before
          );
    END IF;
END;
...
-- ============================================================================
-- Wire check_achievements into process_player_simulation_to_time
-- ============================================================================
-- The function is called at the game-day boundary so achievements are only
-- evaluated once per in-game day (not on every fractional tick).
...
-- Grants and comments
-- ============================================================================
GRANT EXECUTE ON FUNCTION check_achievements(UUID, TIMESTAMPTZ) TO service_role;

COMMENT ON TABLE achievements IS
'Achievement badges unlocked by players through gameplay milestones. One row per achievement type per user.';

COMMENT ON FUNCTION check_achievements(UUID, TIMESTAMPTZ) IS
'Evaluates all achievement conditions for a player and inserts newly unlocked achievements. Called at the game-day boundary in process_player_simulation_to_time.';

COMMENT ON FUNCTION process_player_simulation_to_time(UUID, TIMESTAMP WITH TIME ZONE) IS
'Processes one player simulation tick. Applies active game event multipliers (fuel price, demand), catch-up subsidy for trailing players, cargo revenue (10% baseline scaled by route distance up to 5000km), non-linear aircraft degradation (accelerating wear below 60% condition), and achievement checks at game-day boundaries.';


-- ============================================================================
-- Validation: ensure SQL parses cleanly
-- ============================================================================
-- This migration is idempotent via CREATE OR REPLACE, IF NOT EXISTS, and
-- ON CONFLICT DO NOTHING.
