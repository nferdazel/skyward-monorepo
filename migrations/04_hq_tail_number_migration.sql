-- ==========================================================
-- SKYWARD SIMULATION - SYSTEM RECOVERY MIGRATION SQL
-- ==========================================================

-- 1. ADD BUFFERED COLUMNS TO USERS TO SOLVE LEDGER RECORD CONSOLIDATION BUG
ALTER TABLE users ADD COLUMN IF NOT EXISTS buffered_revenue NUMERIC(20,2) DEFAULT 0.00;
ALTER TABLE users ADD COLUMN IF NOT EXISTS buffered_ops_cost NUMERIC(20,2) DEFAULT 0.00;
ALTER TABLE users ADD COLUMN IF NOT EXISTS buffered_lease_cost NUMERIC(20,2) DEFAULT 0.00;

-- 2. ENSURE AUTO-GROUNDING THRESHOLDS & HQ AIRPORT COLUMNS EXIST
...
-- 3. ENSURE USER_FLEET REFERS TO COMPETITORS & USER WITH ON DELETE CASCADE
ALTER TABLE user_fleet DROP CONSTRAINT IF EXISTS user_fleet_ai_competitor_id_fkey;
ALTER TABLE user_fleet ADD CONSTRAINT user_fleet_ai_competitor_id_fkey 
    FOREIGN KEY (ai_competitor_id) REFERENCES ai_competitors(id) ON DELETE CASCADE;

ALTER TABLE user_routes DROP CONSTRAINT IF EXISTS user_routes_ai_competitor_id_fkey;
ALTER TABLE user_routes ADD CONSTRAINT user_routes_ai_competitor_id_fkey 
    FOREIGN KEY (ai_competitor_id) REFERENCES ai_competitors(id) ON DELETE CASCADE;

-- 4. MASTER HQ TAIL REGISTRATION PREFIX GENERATION FUNCTIONS
...
-- 5. TRIGGER TO LINK FLEET REGISTRATION CODES AUTOMATICALLY TO chosen HQ AIRPORT CHANGE
...
BEGIN
    IF OLD.hq_airport_iata IS DISTINCT FROM NEW.hq_airport_iata THEN
        FOR r_aircraft IN SELECT id FROM user_fleet WHERE user_id = NEW.id LOOP
...
-- 6. RE-ENGINEERED LOOP TRANSACTION CONSOLIDATION ENGINE
