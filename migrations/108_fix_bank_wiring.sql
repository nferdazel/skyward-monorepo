-- ============================================================================
-- Migration 108: Fix bank_transactions write path and wire financing payments
-- ============================================================================
-- 1. take_loan: add bank_transactions INSERT (bot + player paths)
-- 2. repay_loan: add bank_transactions INSERT
-- 3. process_player_simulation_to_time: wire process_aircraft_financing_payments
-- 4. Verify deposit_to_savings / withdraw_from_savings already correct
-- 5. Verify accrue_savings_interest already correct
-- ============================================================================


-- ============================================================================
-- 1. Fix take_loan — add bank_transactions INSERT
-- ============================================================================
...
BEGIN
    ...
    IF v_actor_type = 'AI' THEN
        ...
        SELECT ba.id, v_user_id, 'disbursement', p_principal, ba.balance,
            'Loan disbursement',
            'loan', v_loan_id, v_game_time
        FROM bank_accounts ba
        WHERE ba.user_id = v_user_id AND ba.account_type = 'checking'
        LIMIT 1;
...
-- ============================================================================
-- 2. Fix repay_loan — add bank_transactions INSERT
-- ============================================================================
...
-- ============================================================================
-- 3. Wire process_aircraft_financing_payments into simulation tick
-- ============================================================================
...
BEGIN
    ...
    IF date_trunc('day', p_target_game_time) > date_trunc('day', r_user.game_current_time) THEN
        ...
        -- Process aircraft financing payments
...
COMMENT ON FUNCTION process_player_simulation_to_time(UUID, TIMESTAMPTZ) IS
    'Process player simulation to target game time. Includes credit score, savings interest, and aircraft financing at day boundary.';


-- ============================================================================
-- 4 & 5. Verify: deposit_to_savings, withdraw_from_savings, accrue_savings_interest
--         Already write to bank_transactions correctly (migration 98). No changes needed.
-- ============================================================================

-- Verification queries
DO $$
BEGIN
    -- Verify take_loan writes to bank_transactions
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'take_loan'
          AND pg_get_functiondef(oid) LIKE '%bank_transactions%'
    ) THEN
        RAISE EXCEPTION 'take_loan does not write to bank_transactions';
    END IF;

    -- Verify repay_loan writes to bank_transactions
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'repay_loan'
          AND pg_get_functiondef(oid) LIKE '%bank_transactions%'
    ) THEN
        RAISE EXCEPTION 'repay_loan does not write to bank_transactions';
    END IF;

    -- Verify process_aircraft_financing_payments is called from simulation tick
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'process_player_simulation_to_time'
          AND pg_get_functiondef(oid) LIKE '%process_aircraft_financing_payments%'
    ) THEN
        RAISE EXCEPTION 'process_player_simulation_to_time does not call process_aircraft_financing_payments';
    END IF;

    -- Verify deposit_to_savings writes to bank_transactions
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'deposit_to_savings'
          AND pg_get_functiondef(oid) LIKE '%bank_transactions%'
    ) THEN
        RAISE EXCEPTION 'deposit_to_savings does not write to bank_transactions';
    END IF;

    -- Verify withdraw_from_savings writes to bank_transactions
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'withdraw_from_savings'
          AND pg_get_functiondef(oid) LIKE '%bank_transactions%'
    ) THEN
        RAISE EXCEPTION 'withdraw_from_savings does not write to bank_transactions';
    END IF;

    -- Verify accrue_savings_interest writes to bank_transactions
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'accrue_savings_interest'
          AND pg_get_functiondef(oid) LIKE '%bank_transactions%'
    ) THEN
        RAISE EXCEPTION 'accrue_savings_interest does not write to bank_transactions';
    END IF;

    RAISE NOTICE 'Migration 108 complete: bank_transactions wiring verified.';
END $$;
