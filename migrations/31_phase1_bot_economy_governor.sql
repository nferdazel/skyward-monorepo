-- ============================================================================
-- SKYWARD PHASE 1 BOT ECONOMY GOVERNOR
-- ============================================================================
-- Tightens bot balance around cash-growth realism:
--   1. Expansion requires cash reserve plus lease-burden headroom.
--   2. Distress handling starts from actual lease pressure, not only a flat cash
--      number.
--   3. Archetype pricing/schedule targets are nudged closer to the new route
--      demand model introduced in migration 30.
-- ============================================================================
...
BEGIN
    ...
    FOR r_bot IN SELECT * FROM ai_competitors LOOP
        ...
        v_distress_cash_floor := GREATEST(3000000.00, COALESCE(v_monthly_lease_burden, 0.00) * 2.00);
        ...
        CASE r_bot.archetype
            WHEN 'Regional' THEN
                ...
                v_growth_chance := 0.18;
                v_target_distance := 900.0;
                v_target_price_multiplier := 0.94;
                v_target_schedule_ratio := 0.68;
            WHEN 'Aggressive' THEN
                v_target_fleet_cap := 14;
                v_min_cash_reserve := 6000000.00;
                v_growth_chance := 0.22;
                v_target_distance := 1800.0;
                v_target_price_multiplier := 0.99;
                v_target_schedule_ratio := 0.78;
            ELSE
                v_target_fleet_cap := 10;
                v_min_cash_reserve := 8500000.00;
                v_growth_chance := 0.12;
                v_target_distance := 4200.0;
                v_target_price_multiplier := 1.08;
                v_target_schedule_ratio := 0.52;
        END CASE;
        ...
        IF v_bot_cash < v_distress_cash_floor
           OR COALESCE(r_bot.consecutive_negative_days, 0) >= 2 THEN
            ...
            ORDER BY
                (r.ticket_price / NULLIF(calculate_route_base_fare(r.distance_km), 0.00)) DESC,
        ...
        IF v_fleet_count < v_target_fleet_cap
           AND v_bot_cash > (v_min_cash_reserve + (COALESCE(v_monthly_lease_burden, 0.00) * 1.25))
           ...
           AND random() < v_growth_chance THEN
            ...
            IF v_model_id IS NOT NULL
               AND v_bot_cash >= (v_deposit_amount + (COALESCE(v_monthly_lease_burden, 0.00) * 0.25)) THEN
