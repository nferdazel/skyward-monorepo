-- ============================================================================
-- REFINE OWNER OPTIMIZER RANKING
-- ============================================================================
-- 1. Excludes already-operated routes by default.
-- 2. Deduplicates repeated airframes with identical model/route/seat plans.
-- 3. Applies a simple served-route penalty when existing routes are included.
-- ============================================================================

DROP FUNCTION IF EXISTS public.get_owner_route_optimizer(UUID, VARCHAR, VARCHAR, INT, BOOLEAN);
...
CREATE OR REPLACE FUNCTION public.get_owner_route_optimizer(
    ...
    p_exclude_existing_routes BOOLEAN DEFAULT TRUE
)
RETURNS TABLE (
    aircraft_id UUID,
    tail_number VARCHAR,
...
BEGIN
    ...
    seat_presets AS (
        ...
        ) AS seat_profile(preset_economy_seats, preset_business_seats, preset_first_class_seats)
    ),
    fare_points AS (
        SELECT
            sp.*,
    ...
    ranked AS (
        SELECT
            ...
            ROUND(
                (
                    ((s.computed_expected_passengers_per_flight * s.evaluated_ticket_price) - s.computed_direct_cost_per_flight) *
                    s.computed_weekly_flights *
                    CASE
                        WHEN s.candidate_route_already_exists THEN 0.72
                        ELSE 1.00
                    END
                )::NUMERIC,
                2
            ) AS adjusted_weekly_contribution,
            ROUND(
                CASE
            ...
            ROW_NUMBER() OVER (
                PARTITION BY
                    ...
                    s.evaluated_ticket_price
                ORDER BY
                    s.candidate_currently_assigned ASC,
                    s.candidate_tail_number ASC,
                    s.candidate_aircraft_id ASC
            ) AS route_model_rank
        FROM scored s
        WHERE s.computed_weekly_flights > 0
          AND (
              NOT p_exclude_existing_routes
              OR NOT s.candidate_route_already_exists
          )
    )
    SELECT
        r.candidate_aircraft_id,
        r.candidate_tail_number,
        r.candidate_model_name,
        r.candidate_acquisition_type,
        r.candidate_currently_assigned,
        r.origin_iata,
        r.destination_iata,
        r.candidate_route_already_exists,
        r.route_distance_km,
        r.evaluated_ticket_price,
        r.computed_weekly_flights,
        r.preset_economy_seats,
        r.preset_business_seats,
        r.preset_first_class_seats,
        r.passenger_capacity,
        r.computed_expected_passengers_per_flight,
        r.computed_load_factor,
        r.computed_direct_cost_per_flight,
        r.computed_revenue_per_flight,
        r.computed_contribution_per_flight,
        r.adjusted_weekly_contribution,
        r.computed_maintenance_impact_per_week
    FROM ranked r
    WHERE r.route_model_rank = 1
    ORDER BY
        r.adjusted_weekly_contribution DESC,
        r.computed_contribution_per_flight DESC,
        r.computed_load_factor DESC,
        r.route_distance_km ASC
    LIMIT LEAST(GREATEST(COALESCE(p_limit, 25), 1), 100);
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = public, pg_catalog;

REVOKE ALL ON FUNCTION public.get_owner_route_optimizer(UUID, VARCHAR, VARCHAR, INT, BOOLEAN, BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_owner_route_optimizer(UUID, VARCHAR, VARCHAR, INT, BOOLEAN, BOOLEAN) TO service_role;

COMMENT ON FUNCTION public.get_owner_route_optimizer(UUID, VARCHAR, VARCHAR, INT, BOOLEAN, BOOLEAN) IS
'Returns deduplicated ranked route, fare, and cabin-layout opportunities for the owner, excluding existing routes by default and applying a simple served-route penalty when included.';
