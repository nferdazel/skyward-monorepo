-- ============================================================================
-- Migration 116: Financial Architecture Fix
-- Every financial event writes to BOTH financial_ledger (GL) and
-- bank_transactions (Cash Journal). Also removes dead buffered_* columns.
-- ============================================================================

BEGIN;

-- ── 1. Simulation tick: bank_transactions at day boundary ──────────────────
    ...
    IF v_elapsed_days >= 1.0 THEN
        ...
        IF v_net != 0 THEN
            ...
            SELECT ba.id, p_user_id,
                ...
                v_net,
                (SELECT cash FROM users WHERE id = p_user_id),
...
-- ── 2. Bot simulation: remove buffered_* writes ───────────────────────────
...
-- ── 3. purchase_aircraft: add bank_transactions write ──────────────────────
...
-- ── 4. lease_aircraft: add bank_transactions write ─────────────────────────
...
-- ── 5. sell_aircraft: add bank_transactions write ──────────────────────────
...
-- ── 6. repair_aircraft: add bank_transactions write ────────────────────────
...
-- ── 7. terminate_aircraft_lease: add bank_transactions write ───────────────
...
-- ── 8. finance_aircraft: add bank_transactions write ───────────────────────
...
-- ── 9. process_loan_payments: add bank_transactions for real users ─────────
...
-- ── 10. process_aircraft_financing_payments: add bank_transactions ─────────
...
-- ── 11. take_loan: add financial_ledger write (missing for real users) ─────
...
-- ── 12. repay_loan: add financial_ledger write ─────────────────────────────
...
-- ── 13. accrue_savings_interest: add financial_ledger write ────────────────
...
BEGIN
    ...
    FOR r_account IN
        ...
        WHERE ba.user_id = p_user_id AND ba.account_type = 'savings' AND ba.balance > 0
    LOOP
        v_new_rate := 0.01;
        IF v_config IS NOT NULL THEN
            FOR v_tier IN SELECT jsonb_array_elements(v_config->'tiers') LOOP
                IF r_account.balance >= (v_tier->>'min_balance')::NUMERIC
                   AND (v_tier->>'max_balance' IS NULL OR r_account.balance < (v_tier->>'max_balance')::NUMERIC)
                THEN
        ...
        IF v_interest > 0 THEN
            UPDATE bank_accounts SET balance = balance + v_interest, updated_at = NOW() WHERE id = r_account.id;
            UPDATE users SET cash = cash + v_interest WHERE id = r_account.user_id;

            INSERT INTO bank_transactions (account_id, user_id, transaction_type, amount, balance_after, description, game_date)
            VALUES (r_account.id, r_account.user_id, 'interest', v_interest,
                    r_account.balance + v_interest,
                    'Daily interest accrual (' || (v_new_rate * 100)::TEXT || '% APY)',
                    p_game_date);

            INSERT INTO financial_ledger (user_id, transaction_type, category, amount, description, game_date)
            VALUES (r_account.user_id, 'revenue', 'interest_income', v_interest,
                    'Daily interest accrual (' || (v_new_rate * 100)::TEXT || '% APY)',
                    p_game_date);
        END IF;
    END LOOP;
...
-- ── 14. reset_user_airline: remove buffered_* references ───────────────────
...
-- ── 15. Drop dead buffered_* columns ───────────────────────────────────────
...
-- ── Verification ───────────────────────────────────────────────────────────
SELECT 'financial_ledger' AS tbl, COUNT(*) FROM financial_ledger
