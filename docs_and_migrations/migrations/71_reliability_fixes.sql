...
-- RELIABILITY FIXES
-- ============================================================================
-- Fixes two data-integrity / reliability issues discovered in the codebase.
--
-- Fix 1: sell_aircraft operation ordering
--   Previously the order was: credit cash → write ledger → delete fleet row.
--   If the DELETE failed (FK violation, transient error, etc.), the cash
--   credit and ledger entry persisted — the user received funds for an
--   aircraft that was never actually removed.
--   Fix: delete the fleet row first, then credit cash and write the ledger
--   entry only after the DELETE succeeds.
--
-- Fix 2: tail number collision retry in execute_bot_decisions
--   Bot fleet expansion generates a random tail number via
--   generate_tail_number() and immediately INSERTs into user_fleet.  The
--   tail_number column has a UNIQUE constraint, so a collision causes the
--   INSERT to fail silently — the bot loses its growth opportunity.
--   Fix: wrap the INSERT in a retry loop (max 10 attempts), catching
--   unique_violation each time and regenerating the tail number.
-- ============================================================================


-- ============================================================================
-- FIX 1: sell_aircraft — delete fleet row before crediting cash
-- ============================================================================
...
BEGIN
    ...
    -- Step 1: Delete fleet row FIRST so the aircraft is removed before any
    -- cash movement.  If this fails the transaction aborts cleanly with no
    -- funds credited.
    DELETE FROM user_fleet
    WHERE id = p_fleet_id
      AND user_id = p_user_id;

    -- Step 2: Credit cash to the seller.
    UPDATE users
...
-- ============================================================================
-- FIX 2: execute_bot_decisions — tail number collision retry loop
-- ============================================================================
...
DECLARE
    ...
    v_negative_days INT;
    -- Retry loop state for tail number generation
    v_attempts INT;
    v_inserted BOOLEAN;
BEGIN
    ...
    FOR r_bot IN SELECT * FROM ai_competitors LOOP
        ...
        -- Tail number generation is wrapped in a retry loop to handle
        -- UNIQUE constraint collisions on the tail_number column.
        IF v_fleet_count < v_target_fleet_cap
           ...
           AND random() < v_growth_chance THEN
            ...
            IF v_model_id IS NOT NULL AND v_bot_cash >= v_deposit_amount THEN
                -- Retry loop: generate a fresh tail number on each attempt
                -- and catch unique_violation if the number already exists.
                v_attempts := 0;
                v_inserted := false;
                ...
                WHILE v_attempts < 10 AND NOT v_inserted LOOP
                    ...
                    EXCEPTION WHEN unique_violation THEN
                        -- Tail number collided with an existing row.
                        -- Loop will retry with a newly generated number.
                ...
                ELSE
                    RAISE WARNING 'Could not generate unique tail number for bot % after 10 attempts, skipping fleet expansion', r_bot.id;
                END IF;
            END IF;
        END IF;
