...
- **Migrations are sequential**: `migrations/00_baseline.sql` …
  `migrations/24_seed_active_season.sql`. Apply pending ones with `make migrate`.
- **RLS is off** on every `public` table and there are no policies: authorization
  lives in the Go API (JWT middleware + `user_id` predicates). `01` turned RLS on
  for the Supabase era; `21` turns it back off so a migrated database matches
  prod. Do not rely on RLS for tenant isolation.
...
Data access is split between `internal/store` (reads) and `internal/engine`
(mutations):

- `internal/engine/*` run raw SQL against tables (`users`, `bank_accounts`,
  `bank_transactions`, `fleet_aircraft`, `route_assignments`, `loans`,
  `season_clock`, `finance_snapshots`, …), opening and committing their own
  transaction per mutation (`Pool.Begin` / `Commit`, rolling back on error).
  The package doc is explicit: *"Semua mutasi engine berjalan dalam satu
  transaksi."* — engine mutations use direct table SQL, not the legacy RPC
  functions.
- Uang dibulatkan ke sen di pintu masuk ledger (`LedgerService.applyTx`), dan
  perbandingan terima/tolak memakai helper di `internal/engine/money.go`; kolom
  uang adalah `numeric(20,2)`.
...
| `19_schema_migrations.sql` | Applied-migrations ledger: `schema_migrations` (filename, checksum, applied_at), backfilled with 00–18. Lets `scripts/migrate.sh` apply only pending files and detect edits to applied ones | `schema_migrations` |
| `20_reference_data_seed.sql` | Seed reference data (446 `airports`, 65 `aircraft_models`) so a database built from migrations alone is usable; `ON CONFLICT DO NOTHING`, generated by `scripts/dump-reference-data.sh` | `airports`, `aircraft_models` |
| `21_disable_rls_to_match_prod.sql` | Disable RLS on the 14 tables enabled by `01` and drop its 15 `auth.uid()` policies so a migrated database matches prod (RLS off, zero policies); self-verifying — raises if any RLS or policy survives | `users`, `bank_accounts`, `bank_transactions`, `fleet_aircraft`, `route_assignments`, `loans`, `credit_scores`, `credit_score_history`, `achievements`, `aircraft_models`, `airports`, `game_config`, `season_clock`, `bot_profiles` |
| `22_role_provisioning.sql` | Bootstrap the roles a fresh cluster lacks: a `postgres` role (the baseline dump is a Supabase dump and fails with `role "postgres" does not exist` when `POSTGRES_USER` is named differently) and the `skyward_app` login role the Go API connects as, plus its grants and default ACLs. Idempotent | roles `postgres`, `skyward_app`; ACLs on all `public` tables |
| `24_seed_active_season.sql` | Seed one `active` `season_clock` row so a database built from migrations alone actually works. Its absence made `POST /auth/register` return 500 and the world tick fail every minute with "no active season". Idempotent via `WHERE NOT EXISTS` (the `season_clock_one_active_idx` allows only one active row) | `season_clock` |
| `23_reconcile_fk_constraints.sql` | Restore three foreign keys that `00_baseline.sql` declares but prod is missing (found by `scripts/drift-check.sh`, item 0.3b): `users.hq_airport_iata → airports`, `users.season_id → season_clock`, `world_tick_log.season_id → season_clock ON DELETE CASCADE`. Prod data violates none, so adding them is safe | `users`, `world_tick_log` |
...
- **Most SQL functions in `00_baseline.sql` are dead.** The dump carries ~113
  PL/pgSQL functions from the Supabase era; only 13 are reachable today (4
  called from Go: `get_hq_prefix`, `normalize_username`, `get_config_numeric`,
  `haversine_distance`; 9 via the 5 live triggers). Named in the file's header
  with the evidence. Do not read `process_world_tick` / `take_loan` / etc. in
  there as current behaviour — the Go engine is authoritative, and they are
  kept rather than dropped because pruning them cannot be proven safe from code
  alone.
