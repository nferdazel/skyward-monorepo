...
CREATE FUNCTION public.apply_actor_bankruptcy_state(p_user_id uuid) RETURNS void
...
END;
$$;
CREATE FUNCTION public.assign_actor_aircraft_to_route(p_user_id uuid, p_route_id uuid, p_aircraft_id uuid) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.assign_aircraft_to_route(p_route_id uuid, p_aircraft_id uuid) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql
    AS $$
...
END;
$$;
CREATE FUNCTION public.assign_aircraft_to_route(p_user_id uuid, p_route_id uuid, p_aircraft_id uuid) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
END;
$$;
CREATE FUNCTION public.bot_handle_financial(p_bot_id uuid, p_game_time timestamp with time zone, p_distress character varying, p_bot_cash numeric, p_starting_cash numeric, p_min_cash_reserve numeric, p_repay_ratio numeric, p_recovery_amount numeric) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.bot_handle_pricing(p_bot_id uuid, p_game_time timestamp with time zone, p_archetype character varying, p_distress character varying, p_target_price_mult numeric, p_comp_threshold numeric) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.bot_handle_repair(p_bot_id uuid, p_game_time timestamp with time zone, p_distress character varying, p_threshold numeric, p_cash_reserve numeric) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.bot_handle_route_lifecycle(p_bot_id uuid, p_game_time timestamp with time zone, p_distress character varying, p_target_price_mult numeric, p_loss_days_thresh integer) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.build_synthetic_auth_email(p_username text) RETURNS text
...
CREATE FUNCTION public.calculate_airport_congestion_factor(p_origin_iata character varying) RETURNS numeric
...
END;
$$;
CREATE FUNCTION public.calculate_airport_demand_factor(p_origin_demand integer, p_destination_demand integer) RETURNS numeric
    LANGUAGE sql STABLE
    AS $$
    SELECT GREATEST(
...
$$;
CREATE FUNCTION public.calculate_credit_score(p_user_id uuid) RETURNS TABLE(total_score integer, tier character varying, fleet_health integer, revenue_stability integer, debt_ratio integer, cash_reserve integer, profit_history integer)
...
CREATE FUNCTION public.calculate_hub_bonus(p_origin_iata character varying, p_user_id uuid) RETURNS numeric
    LANGUAGE plpgsql STABLE
    AS $$
DECLARE
...
CREATE FUNCTION public.calculate_lease_termination_fee(p_lease_price_per_month numeric) RETURNS numeric
    LANGUAGE sql IMMUTABLE
    AS $$
    SELECT ROUND(COALESCE(p_lease_price_per_month, 0.00) * 0.25, 2);
$$;
CREATE FUNCTION public.calculate_required_lease_deposit(p_purchase_price numeric, p_lease_price_per_month numeric) RETURNS numeric
    LANGUAGE plpgsql STABLE
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.calculate_route_base_fare(p_distance_km double precision) RETURNS numeric
    LANGUAGE sql STABLE
    AS $$
...
$$;
CREATE FUNCTION public.calculate_route_demand_multiplier(p_distance_km double precision, p_ticket_price numeric) RETURNS numeric
    LANGUAGE sql IMMUTABLE
    AS $$
    SELECT GREATEST(
        0.00,
...
$$;
CREATE FUNCTION public.calculate_route_expected_passengers(p_capacity integer, p_distance_km double precision, p_ticket_price numeric, p_origin_demand integer, p_destination_demand integer) RETURNS integer
    LANGUAGE sql IMMUTABLE
    AS $$
    SELECT GREATEST(
        0,
...
$$;
CREATE FUNCTION public.calculate_route_expected_passengers(p_capacity integer, p_distance_km double precision, p_ticket_price numeric, p_origin_demand integer, p_destination_demand integer, p_origin_iata character varying, p_destination_iata character varying, p_user_id uuid) RETURNS integer
    LANGUAGE plpgsql STABLE
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.calculate_route_max_weekly_flights(p_distance_km double precision, p_speed_kmh integer) RETURNS integer
    LANGUAGE sql STABLE
    AS $$
...
$$;
CREATE FUNCTION public.calculate_route_max_weekly_flights(p_distance_km double precision, p_speed_kmh integer, p_turnaround_hours numeric) RETURNS integer
    LANGUAGE sql STABLE
    AS $$
    SELECT CASE WHEN COALESCE(p_distance_km, 0.0) <= 0.0 OR COALESCE(p_speed_kmh, 0) <= 0 THEN 0
...
CREATE FUNCTION public.calculate_user_net_worth(p_user_id uuid) RETURNS numeric
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.check_achievements(p_user_id uuid, p_game_time timestamp with time zone) RETURNS TABLE(achievement_name character varying, achievement_type character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $_$
DECLARE
...
CREATE FUNCTION public.configure_aircraft_seats(p_fleet_id uuid, p_economy_seats integer, p_business_seats integer, p_first_class_seats integer) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.configure_aircraft_seats(p_user_id uuid, p_fleet_id uuid, p_economy_seats integer, p_business_seats integer, p_first_class_seats integer) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
END;
$$;
CREATE FUNCTION public.create_route(p_origin_iata character varying, p_destination_iata character varying, p_distance_km numeric, p_ticket_price numeric, p_flights_per_week integer) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.create_route(p_user_id uuid, p_origin_iata character varying, p_destination_iata character varying, p_distance_km numeric, p_ticket_price numeric, p_flights_per_week integer) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
END;
$$;
CREATE FUNCTION public.credit_bank_account(p_user_id uuid, p_amount numeric, p_ifrs_category character varying, p_ifrs_subcategory character varying, p_description text, p_game_date timestamp with time zone) RETURNS numeric
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.deactivate_expired_events(p_game_time timestamp with time zone) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
    UPDATE game_events
...
CREATE FUNCTION public.debit_bank_account(p_user_id uuid, p_amount numeric, p_ifrs_category character varying, p_ifrs_subcategory character varying, p_description text, p_game_date timestamp with time zone) RETURNS numeric
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.delete_account() RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.delete_route(p_route_id uuid) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.delete_route(p_user_id uuid, p_route_id uuid) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
END;
$$;
CREATE FUNCTION public.ensure_world_current(p_season_id uuid DEFAULT NULL::uuid) RETURNS TABLE(season_id uuid, ticks_processed integer, game_time_after timestamp with time zone, players_processed integer, bots_processed integer)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_season_id UUID;
...
END;
$$;
CREATE FUNCTION public.execute_bot_decisions() RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.finance_aircraft(p_aircraft_model_id uuid, p_down_payment_pct numeric DEFAULT 0.20, p_term_months integer DEFAULT 36) RETURNS TABLE(success boolean, message text, new_cash numeric)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
CREATE FUNCTION public.finance_aircraft(p_user_id uuid, p_aircraft_model_id uuid, p_down_payment_pct numeric DEFAULT 0.20, p_term_months integer DEFAULT 36) RETURNS TABLE(success boolean, message text, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $_$
DECLARE
...
CREATE FUNCTION public.generate_ceo_name() RETURNS character varying
    LANGUAGE plpgsql
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.generate_company_name(p_archetype character varying) RETURNS character varying
    LANGUAGE plpgsql
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.generate_game_events(p_game_time timestamp with time zone) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_roll NUMERIC;
...
CREATE FUNCTION public.generate_tail_number(p_airport_iata character varying) RETURNS character varying
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_prefix VARCHAR;
...
END;
$$;
CREATE FUNCTION public.get_bot_health() RETURNS TABLE(bot_id uuid, username character varying, archetype character varying, bot_status character varying, cash numeric, fleet_count bigint, route_count bigint)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
END;
$$;
CREATE FUNCTION public.get_credit_tier_policy(p_tier character varying) RETURNS jsonb
    LANGUAGE plpgsql STABLE
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.get_database_size_report() RETURNS TABLE(metric text, value text)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.get_finance_snapshot(p_id uuid, p_is_bot boolean DEFAULT false) RETURNS TABLE(actor_id uuid, is_bot boolean, company_name character varying, cash numeric, net_worth numeric, owned_aircraft_asset_value numeric, leased_aircraft_monthly_exposure numeric, fleet_count integer, owned_fleet_count integer, leased_fleet_count integer, active_route_count integer, rolling_revenue_30d numeric, rolling_expense_30d numeric, rolling_net_30d numeric, ledger_window_days integer)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.get_global_leaderboard() RETURNS TABLE(id uuid, company_name character varying, ceo_name character varying, is_bot boolean, archetype character varying, cash numeric, net_worth numeric, fleet_size integer, monthly_revenue numeric, status character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
END;
$$;
CREATE FUNCTION public.get_hq_prefix(p_airport_iata character varying) RETURNS character varying
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_country VARCHAR;
...
CREATE FUNCTION public.get_route_performance(p_user_id uuid) RETURNS TABLE(route_id uuid, origin_iata character varying, destination_iata character varying, distance_km double precision, ticket_price numeric, flights_per_week integer, assigned_aircraft character varying, effective_capacity integer, expected_passengers integer, load_factor numeric, revenue_per_flight numeric, cost_per_flight numeric, profit_per_flight numeric, weekly_profit numeric)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.get_table_size_report() RETURNS TABLE(schema_name text, table_name text, row_estimate bigint, total_size_bytes bigint, total_size_pretty text, table_size_pretty text, index_size_pretty text)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
BEGIN
...
CREATE FUNCTION public.get_tail_suffix(p_tail character varying) RETURNS character varying
    LANGUAGE plpgsql
    AS $$
BEGIN
...
END;
$$;
CREATE FUNCTION public.get_user_balance(p_user_id uuid) RETURNS numeric
    LANGUAGE sql STABLE
    AS $$
    ...
    LIMIT 1;
$$;
CREATE FUNCTION public.get_user_id_for_auth_uid(p_auth_user_id uuid DEFAULT auth.uid()) RETURNS uuid
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
    SELECT u.id
...
CREATE FUNCTION public.get_world_tick_guardrail_report() RETURNS TABLE(check_name text, check_status text, details text)
    LANGUAGE plpgsql STABLE
    AS $$
DECLARE
    r_season RECORD;
...
END;
$$;
CREATE FUNCTION public.haversine_distance(lat1 numeric, lon1 numeric, lat2 numeric, lon2 numeric) RETURNS numeric
    LANGUAGE plpgsql IMMUTABLE
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.lease_aircraft(p_model_id uuid, p_nickname character varying, p_economy_seats integer DEFAULT NULL::integer, p_business_seats integer DEFAULT 0, p_first_class_seats integer DEFAULT 0) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.lease_aircraft(p_user_id uuid, p_model_id uuid, p_nickname character varying, p_economy_seats integer DEFAULT NULL::integer, p_business_seats integer DEFAULT 0, p_first_class_seats integer DEFAULT 0) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $_$
DECLARE
...
CREATE FUNCTION public.perform_actor_aircraft_repair(p_user_id uuid, p_fleet_id uuid, p_min_cash_reserve numeric DEFAULT 0, p_game_time timestamp with time zone DEFAULT NULL::timestamp with time zone, p_description text DEFAULT NULL::text) RETURNS TABLE(success boolean, message character varying, new_cash numeric, repair_cost numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $_$
DECLARE
...
END;
$_$;
CREATE FUNCTION public.process_actor_day_boundary(p_user_id uuid, p_game_date timestamp with time zone, p_elapsed_days numeric DEFAULT 1.0) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.process_aircraft_financing_payments(p_user_id uuid, p_game_date timestamp with time zone) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.process_all_bots_simulation_to_time(p_target_game_time timestamp with time zone, p_season_id uuid DEFAULT NULL::uuid) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.process_credit_at_day_boundary(p_user_id uuid, p_game_date timestamp with time zone) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
BEGIN
...
END;
$$;
CREATE FUNCTION public.process_loan_payments(p_user_id uuid, p_game_date timestamp with time zone) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
DECLARE
...
CREATE FUNCTION public.process_player_simulation_to_time(p_user_id uuid, p_target_game_time timestamp with time zone) RETURNS TABLE(game_time timestamp with time zone, cash numeric, flights_run integer, elapsed_days numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.process_simulation_delta() RETURNS TABLE(cash_before numeric, cash_after numeric, elapsed_real_sec double precision, elapsed_game_days double precision, flights_run integer)
    LANGUAGE plpgsql SECURITY DEFINER
...
END;
$$;
CREATE FUNCTION public.process_simulation_delta(p_user_id uuid) RETURNS TABLE(cash_before numeric, cash_after numeric, elapsed_real_sec double precision, elapsed_game_days double precision, flights_run integer)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
DECLARE
...
CREATE FUNCTION public.process_world_tick(p_season_id uuid DEFAULT NULL::uuid, p_max_ticks integer DEFAULT 10) RETURNS TABLE(season_id uuid, ticks_processed integer, game_time_after timestamp with time zone, players_processed integer, bots_processed integer)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.prune_bank_transactions(p_dry_run boolean DEFAULT true) RETURNS TABLE(action text, detail text, row_count bigint)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.prune_world_tick_log() RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.purchase_aircraft(p_model_id uuid, p_nickname character varying, p_economy_seats integer DEFAULT NULL::integer, p_business_seats integer DEFAULT 0, p_first_class_seats integer DEFAULT 0) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.purchase_aircraft(p_user_id uuid, p_model_id uuid, p_nickname character varying, p_economy_seats integer DEFAULT NULL::integer, p_business_seats integer DEFAULT 0, p_first_class_seats integer DEFAULT 0) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.refinance_loan(p_loan_id uuid) RETURNS TABLE(success boolean, message text, new_rate numeric, savings numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.repair_aircraft(p_fleet_id uuid) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.repair_aircraft(p_user_id uuid, p_fleet_id uuid) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.repay_loan(p_loan_id uuid, p_amount numeric DEFAULT NULL::numeric) RETURNS TABLE(success boolean, message text, new_cash numeric, paid_off boolean)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $_$
DECLARE
...
END;
$_$;
CREATE FUNCTION public.require_current_user_id() RETURNS uuid
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'auth', 'pg_catalog'
    AS $$
DECLARE
...
CREATE FUNCTION public.reset_user_airline() RETURNS TABLE(success boolean, message text)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.reset_user_airline(p_user_id uuid) RETURNS TABLE(success boolean, message text)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.resolve_active_season_id(p_season_id uuid DEFAULT NULL::uuid) RETURNS uuid
    LANGUAGE plpgsql STABLE
    AS $$
DECLARE
    v_season_id UUID;
...
END;
$$;
CREATE FUNCTION public.resolve_credit_tier(p_score integer) RETURNS character varying
    LANGUAGE plpgsql STABLE
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.save_airline_settings(p_company_name character varying, p_auto_grounding_threshold numeric, p_hq_airport_iata character varying) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.save_airline_settings(p_user_id uuid, p_company_name character varying, p_auto_grounding_threshold numeric, p_hq_airport_iata character varying) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
END;
$$;
CREATE FUNCTION public.sell_actor_aircraft(p_user_id uuid, p_fleet_id uuid, p_game_time timestamp with time zone) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $_$
DECLARE
...
END;
$_$;
CREATE FUNCTION public.sell_aircraft(p_fleet_id uuid) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.sell_aircraft(p_user_id uuid, p_fleet_id uuid) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.spawn_bot() RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.take_loan(p_principal numeric, p_term_weeks integer DEFAULT 52, p_loan_type character varying DEFAULT 'unsecured'::character varying, p_collateral_aircraft_id uuid DEFAULT NULL::uuid) RETURNS TABLE(success boolean, message text, new_cash numeric)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
CREATE FUNCTION public.take_loan(p_user_id uuid, p_principal numeric, p_term_weeks integer DEFAULT 52, p_loan_type character varying DEFAULT 'unsecured'::character varying, p_collateral_aircraft_id uuid DEFAULT NULL::uuid) RETURNS TABLE(success boolean, message text, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $_$
DECLARE
...
CREATE FUNCTION public.terminate_actor_lease(p_user_id uuid, p_fleet_id uuid, p_game_time timestamp with time zone) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.terminate_aircraft_lease(p_fleet_id uuid) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.terminate_aircraft_lease(p_user_id uuid, p_fleet_id uuid) RETURNS TABLE(success boolean, message character varying, new_cash numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
END;
$$;
CREATE FUNCTION public.trg_bank_balance_reconcile_net_worth() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_user_id UUID;
...
END;
$$;
CREATE FUNCTION public.trg_create_default_bank_account() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_starting_cash NUMERIC;
...
END;
$$;
CREATE FUNCTION public.trg_fleet_reconcile_net_worth() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_user_id UUID;
...
END;
$$;
CREATE FUNCTION public.trg_loan_reconcile_net_worth() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_user_id UUID;
...
END;
$$;
CREATE FUNCTION public.trg_sync_tail_numbers_on_hq_change() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
...
END;
$$;
CREATE FUNCTION public.trg_update_user_net_worth() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
END;
$$;
CREATE FUNCTION public.update_credit_score(p_user_id uuid, p_game_date timestamp with time zone) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.update_route_frequency_and_price(p_route_id uuid, p_ticket_price numeric, p_flights_per_week integer) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE v_user_id UUID;
BEGIN
...
END;
$$;
CREATE FUNCTION public.update_route_frequency_and_price(p_user_id uuid, p_route_id uuid, p_ticket_price numeric, p_flights_per_week integer) RETURNS TABLE(success boolean, message character varying)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
CREATE TABLE public.aircraft_models (
    ...
    min_credit_tier text DEFAULT 'Standard'::text NOT NULL,
    CONSTRAINT aircraft_models_type_check CHECK (((type)::text = ANY (ARRAY[('regional_turboprop'::character varying)::text, ('regional_jet'::character varying)::text, ('narrow_body_jet'::character varying)::text, ('wide_body_jet'::character varying)::text])))
);
...
CREATE TABLE public.bank_transactions (
    ...
    CONSTRAINT bank_transactions_transaction_type_check CHECK (((transaction_type)::text = ANY (ARRAY[('debit'::character varying)::text, ('credit'::character varying)::text, ('payment'::character varying)::text, ('deposit'::character varying)::text, ('disbursement'::character varying)::text, ('refinance'::character varying)::text, ('late_fee'::character varying)::text, ('accrual'::character varying)::text, ('refund'::character varying)::text])))
);
CREATE TABLE public.bot_profiles (
    ...
    CONSTRAINT bot_profiles_distress_stage_check CHECK (((distress_stage)::text = ANY (ARRAY[('stable'::character varying)::text, ('cautious'::character varying)::text, ('defensive'::character varying)::text, ('desperate'::character varying)::text])))
);
...
CREATE TABLE public.fleet_aircraft (
    ...
    CONSTRAINT fleet_aircraft_acquisition_type_check CHECK (((acquisition_type)::text = ANY (ARRAY[('purchase'::character varying)::text, ('lease'::character varying)::text, ('finance'::character varying)::text]))),
    CONSTRAINT fleet_aircraft_condition_check CHECK (((condition >= 0.00) AND (condition <= 100.00))),
    CONSTRAINT fleet_aircraft_status_check CHECK (((status)::text = ANY (ARRAY[('grounded'::character varying)::text, ('active'::character varying)::text, ('maintenance'::character varying)::text])))
);
...
CREATE TABLE public.loans (
    ...
    weekly_payment numeric(20,2) NOT NULL,
    ...
    CONSTRAINT loans_loan_type_check CHECK (((loan_type)::text = ANY (ARRAY[('unsecured'::character varying)::text, ('secured'::character varying)::text, ('credit_line'::character varying)::text, ('aircraft_financing'::character varying)::text]))),
    CONSTRAINT loans_principal_check CHECK ((principal > (0)::numeric)),
    CONSTRAINT loans_status_check CHECK (((status)::text = ANY (ARRAY[('active'::character varying)::text, ('paid_off'::character varying)::text, ('defaulted'::character varying)::text, ('repossessed'::character varying)::text])))
);
...
CREATE TABLE public.route_assignments (
    ...
    CONSTRAINT route_assignments_status_check CHECK (((status)::text = ANY (ARRAY[('active'::character varying)::text, ('cancelled'::character varying)::text]))),
...
CREATE TABLE public.season_clock (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT season_clock_status_check CHECK (((status)::text = ANY (ARRAY[('draft'::character varying)::text, ('active'::character varying)::text, ('paused'::character varying)::text, ('completed'::character varying)::text]))),
...
CREATE TABLE public.world_tick_log (
    ...
    message text,
    CONSTRAINT world_tick_log_status_check CHECK (((status)::text = ANY (ARRAY[('started'::character varying)::text, ('skipped'::character varying)::text, ('success'::character varying)::text, ('error'::character varying)::text, ('player_error'::character varying)::text, ('bot_error'::character varying)::text])))
);
...
CREATE SEQUENCE public.world_tick_log_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE ONLY public.finance_snapshots
    ADD CONSTRAINT finance_snapshots_user_id_snapshot_game_time_key UNIQUE (user_id, snapshot_game_time);
...
CREATE INDEX finance_snapshots_user_time_idx ON public.finance_snapshots USING btree (user_id, snapshot_game_time DESC);
...
CREATE TRIGGER create_default_bank_account AFTER INSERT ON public.users FOR EACH ROW EXECUTE FUNCTION public.trg_create_default_bank_account();
CREATE TRIGGER fleet_reconcile_net_worth AFTER INSERT OR DELETE OR UPDATE ON public.fleet_aircraft FOR EACH ROW EXECUTE FUNCTION public.trg_fleet_reconcile_net_worth();
CREATE TRIGGER trg_bank_balance_reconcile_net_worth AFTER INSERT OR DELETE OR UPDATE OF balance, user_id ON public.bank_accounts FOR EACH ROW EXECUTE FUNCTION public.trg_bank_balance_reconcile_net_worth();
CREATE TRIGGER trg_loan_reconcile_net_worth AFTER INSERT OR DELETE OR UPDATE OF remaining_balance, status, user_id ON public.loans FOR EACH ROW EXECUTE FUNCTION public.trg_loan_reconcile_net_worth();
CREATE TRIGGER trg_user_hq_change AFTER UPDATE OF hq_airport_iata ON public.users FOR EACH ROW EXECUTE FUNCTION public.trg_sync_tail_numbers_on_hq_change();
ALTER TABLE ONLY public.achievements
...
ALTER TABLE ONLY public.finance_snapshots
    ADD CONSTRAINT finance_snapshots_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.fleet_aircraft
